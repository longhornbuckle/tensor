...
{
  using type = typename allocator_result< decltype( ::std::declval< LINALG_EXPRESSIONS::conjugate_tensor_expression< Tensor, Transpose > >().underlying() ) >::type;
  [[nodiscard]] static inline constexpr type get_allocator( const LINALG_EXPRESSIONS::conjugate_tensor_expression< Tensor, Transpose >& t ) noexcept
    { return allocator_result< decltype( ::std::declval< LINALG_EXPRESSIONS::conjugate_tensor_expression< Tensor, Transpose > >().underlying() ) >::get_allocator( t.underlying() ); }
};
